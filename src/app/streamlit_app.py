...
if manual_mode:
    info_cols = st.columns([1.2, 1.0, 1.1, 1.1, 1.0])
    market_display = info_cols[0].selectbox("市场类型", ["普通胜平负", "让球胜平负"])
    match_code = info_cols[1].text_input("比赛编号", value="手动001")
    league = info_cols[2].text_input("赛事", value="世界杯")
    kickoff_time = info_cols[3].text_input("开赛时间", value="")
    supports_single = info_cols[4].checkbox("是否单关", value=False)
    ...
    if group:
        ...
        st.info("未匹配本地小组，仅显示赔率市场分析。")

    st.subheader("手动录入赔率")
    st.caption("普通胜平负")
    normal_cols = st.columns(3)
    normal_home_odds = normal_cols[0].number_input("主胜赔率", min_value=1.01, value=1.47, step=0.05)
    normal_draw_odds = normal_cols[1].number_input("平局赔率", min_value=1.01, value=3.70, step=0.05)
    normal_away_odds = normal_cols[2].number_input("主负赔率", min_value=1.01, value=5.60, step=0.05)

    st.caption("让球胜平负")
    handicap_cols = st.columns(4)
    handicap_value = handicap_cols[0].number_input("让球", value=-1, step=1)
    handicap_home_odds = handicap_cols[1].number_input("让胜赔率", min_value=1.01, value=2.56, step=0.05)
    handicap_draw_odds = handicap_cols[2].number_input("让平赔率", min_value=1.01, value=3.30, step=0.05)
    handicap_away_odds = handicap_cols[3].number_input("让负赔率", min_value=1.01, value=2.30, step=0.05)
    ...
    with st.expander("模型概率输入", expanded=False):
        probability_preset = st.selectbox(
            "概率预设",
            ["均衡 33/34/33", "主队优势", "客队优势", "让球保守盘", "自定义"],
            index=1 if market_type == "normal" else 4,
        )
        ...
        model_home = prob_cols[0].number_input(
            ...
            step=0.01,
            key=f"{market_type}_{probability_preset}_manual_model_home",
        )
        model_draw = prob_cols[1].number_input(
            ...
            step=0.01,
            key=f"{market_type}_{probability_preset}_manual_model_draw",
        )
        model_away = prob_cols[2].number_input(
            ...
            step=0.01,
            key=f"{market_type}_{probability_preset}_manual_model_away",
        )
        odds = odds_defaults
